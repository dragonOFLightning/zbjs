...
/**@function ZombieRadar_onEnable 模块启用时调用 */
function ZombieRadar_onEnable() {
...
/**@function ZombieRadar_onUpdate 每tick调用 */
function ZombieRadar_onUpdate() {
...
/**
 * @function ZombieRadar_onPacket 监听到Packet事件时调用
 * @param {net.ccbluex.liquidbounce.event.PacketEvent} event 
 */
function ZombieRadar_onPacket(event) {
    // 获取数据包
...
/**
 * @function ZombieRadar_onRender2D 监听到渲染2D事件时调用
 * @param {net.ccbluex.liquidbounce.event.Render2DEvent} event 
*/
function ZombieRadar_onRender2D(event) {
 ...
 */
function ZombieRadar_onRender3D(event) {
    /// 获取设置的颜色
...
/**@override */
ZombieRadar.prototype.onEnable = ZombieRadar_onEnable
/**@override */
ZombieRadar.prototype.onUpdate = ZombieRadar_onUpdate
/**@override */
ZombieRadar.prototype.onPacket = ZombieRadar_onPacket
/**@override */
ZombieRadar.prototype.onRender2D = ZombieRadar_onRender2D
/**@override */
ZombieRadar.prototype.onRender3D = ZombieRadar_onRender3D
/**@override 覆写添加值函数 用于给模块添加选项*/
ZombieRadar.prototype.addValues = function (values) {
...
function onLoad() {
    ...
    // 文字反馈模块已加载
    chat.print('§9ZombieRadar §2- §4Load')
}
...
function onEnable() {
    ...
    // 注册模块
    scriptModule = moduleManager.registerModule(new ZombieRadar())
}
